...
public class Timer_welldone_3 : MonoBehaviour
{
    ...
    public void StopTime(bool isDead)
    {
        ...
        if (!isDead)
        {
            ...
            if (elapsedTime <= 480) // 3 minutes in seconds
            {
                points.IncrementPoints(30);
            }
            // Debug log if elapsedTime is less than or equal to 5 minutes
            else if (elapsedTime <= 600) // 5 minutes in seconds
            {
                points.IncrementPoints(15);
            }
            else if (elapsedTime <= 720) // 8 minutes in seconds
            {
